...
const Ask_Ques = () => {
    ...
    const addQuestion = async (e) => {
        e.preventDefault()
        await db.add({ title: titles, question: questions, user_address: db.signer() }, "Questions")
                            ...
                            <table className='flex flex-col items-center justify-center h-72'>
                                <tr className='flex flex-row w-full items-center justify-center text-center '>
                                    ...
                                    <td className='rounded-2xl'>
                                        <input type="text" required className='text-xl w-56 lg:w-80 rounded-3xl px-4 py-2 bg-white' placeholder="Write the shortest title as per question" value={titles} onChange={(e) => setTitles(e.target.value)} />
                                    </td>
                                </tr>
                                <br></br>
                                <tr className='flex flex-row w-full items-center justify-center'>
                                    ...
                                    <td className='rounded-3xl text-xl'>
                                        <textarea type="text" required className='w-full lg:w-80 rounded-2xl px-4 py-2 bg-white' placeholder='Explain the whole question in detail here' value={questions} onChange={(e) => setQuestions(e.target.value)} />
                                    </td>
                                </tr>
                            </table>
                            <button className='w-fit flex items-center justify-center font-bold py-5 text-center px-12 rounded-full border-2 border-white hover:bg-black hover:border-blue-700 bg-white text-black hover:text-white duration-700'
                                type='submit'
                                onClick={addQuestion}
                            >
                                SUBMIT
